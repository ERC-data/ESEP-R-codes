...
nameThisWorkOfYours= 'CSIR_solarProfile_8ts'
...
names(loadData)[names(loadData)=='X1'] = 'DataVar' #renaming column
loadData = loadData[loadData$Year <= 2016, c('Date','DataVar')] #get all data for pre 2016

#read in user timeslice designation from excel  
...
if(length(unique(loadData$year))>1){
  print('...there is data for more than one year')
  print('averaging over all years...')
...
if(length(unique(loadData$min))>1){
  print('...resolution is better than an hour (ie. minutely).')
  print('averaging over the hours...')
...
ggsave(paste(saveCSVFilePath,paste(nameThisWorkOfYours,'.png',sep=''),sep ='/'),loadPlot)
